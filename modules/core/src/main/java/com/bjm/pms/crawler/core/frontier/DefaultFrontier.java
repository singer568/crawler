...
import com.bjm.pms.crawler.core.data.queue.SimpleTaskQueue;
import com.bjm.pms.crawler.core.data.queue.TaskQueue;
...
public class DefaultFrontier implements Frontier{
	...
	private void populateTaskQueue(CrawlLinkURI uri,Integer planNum){
		...
		try {
			...
			String html = this.controller.getHttpClient().execute(target, httpGet, this.controller.getHandler(), context);
